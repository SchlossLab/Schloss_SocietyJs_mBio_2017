...
a <- all_data %>%
		...
		ggplot(aes(year, n_papers)) +
			...
			scale_color_manual(breaks=c("TRUE", "FALSE"),
                            labels=c("Microbiology Society Journal", "Other Journal"),
														values=c("red", "blue"))+
...
b <- all_data %>%
		...
		ggplot(aes(year, n_papers)) +
			...
			scale_x_continuous(limits=c(1980,2015), breaks=seq(1980,2015,5)) +
			scale_color_manual(breaks=c("American Society for Microbiology", "Federation of European Microbiological Societies", "Infectious Diseases Society of America", "Microbiology Society", "Society for Applied Microbiology"),
														labels=c("ASM", "FEMS", "IDSCA", "Microbiology Society", "SfAM"),
														values=cbbPalette) +
			ylab("Number of Microbiology Papers Published") +
			xlab("Year") +
			my_theme +
...
d <- mega_journal_data %>%
		ggplot(aes(year, n_papers)) +
			...
			scale_color_manual(breaks=c("Front Microbiol", "PeerJ", "PLoS One", "Sci Rep"),
														labels=c("Frotiers in Microbiology", "PeerJ", "PLOS ONE", "Scientific Reports"),
														values=rev(cbbPalette)) +
			ylab("Number of Microbiology Papers Published") +
			xlab("Year") +
			my_theme +
			theme(
...
ggdraw() +
	...
	ggsave('figures/pub_analysis.eps', width=6.5, height=6.5, unit='in')
